    ...
    def get_Response_time(self,p,Q,B,tbit):
        ...
        for q in range(int(Q)):
            ...
            if q == 0:
                ...
                f_resp= round((prev_inst - (math.floor(q/self.num_frame[p]) *self.period[p]) + (math.floor(((q %self.num_frame[p])+1))/self.num_frame[p]) *self.cmac[p] +((1- math.floor((q %self.num_frame[p])+1)/self.num_frame[p])*self.cmax[p])),6)
                ...
                l_resp = round((new_inst - (math.floor(q/self.num_frame[p]) *self.period[p]) + (math.floor(((q %self.num_frame[p])+1))/self.num_frame[p]) *self.cmac[p] +((1- math.floor((q %self.num_frame[p])+1)/self.num_frame[p])*self.cmax[p])),6)
            if f_resp > l_resp:
